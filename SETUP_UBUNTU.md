...
教育デモの初版では、後述のSSHトンネル方式を標準とする。Tailscaleは複数端末からの接続、常時運用、オーバーレイ側のアクセス制御が必要になった場合に選択する。
...
教育デモを単一の教員ノートPCから実施する場合は、**--overlay none** とSSHローカルポートフォワーディングを使う。
...
この方式ではDebate APIを127.0.0.1だけで待ち受けさせ、SSHの認証とホスト鍵管理を接続制御にする。
