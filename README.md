...
初版ではアンケートやGoogle Drive連携を使わず、Cの最終整理を学生同士の議論へ引き継ぎます。終了後は画面の「PDFをダウンロード」で、CのMarkdown形式の最終整理だけをPDFとして保存できます。
...
- `GET /api/debates/{id}/export.pdf`：完了したディベートをPDFとしてダウンロード
- `GET /health`：サービスとモデルの確認

ターン順はサーバー側で固定し、LLM同時実行数は1に制限します。

初版の終了後は、画面の「Cのまとめを確認」または「学生同士の議論へ」からCの整理を確認し、学生同士の議論へ移行します。
旧版のGoogle Drive・アンケートAPIは初版では無効化しており、外部保存処理は実行しません。
