...
Google Formsの回答取得、参照資料・分析レポートのGoogle Drive保存は
[GOOGLE_WORKSPACE_SETUP.md](GOOGLE_WORKSPACE_SETUP.md) にまとめています。
...
- `POST /api/debates/{id}/reference`：アンケート用参照資料を生成し、Google Driveへ保存
- `POST /api/debates/{id}/survey/start`：回答受付の開始時刻を記録
- `POST /api/debates/{id}/survey/analyze`：Google Form回答を集計・分析し、Google Driveへ保存
